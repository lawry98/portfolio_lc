...
- **NEXT → T2 — Typography, sections, scroll & reveals.** On session start: read `docs/*`, then invoke `superpowers:subagent-driven-development` to execute T2 from `TICKETS.md`. Builds on T1 tokens/theme/shells. ⚠️ Carry-forward: `.hero__line` is `white-space:nowrap` — re-check each dynamic phrase for 390px horizontal overflow. T2 adds the first runtime deps beyond tooling (gsap, lenis, + SplitText) — install + add a `DECISIONS.md` entry.
