...
| 17 | **The visor bottom-rim speckle and the eyes** on a real GPU / DPR 3 phone (T-GLB, R-GLB-19 watch) | Real phone at DPR ≥ 3; look at Byte's eyes (should read as clean, stable mint ovals after the R-GLB-19 depth-bias fix) and the visor's bottom rim (a faint residual speckle from the same depth-precision cause is expected and accepted) | R-GLB-19: `GLOW_DEPTH_BIAS` (`polygonOffset` factor −1, units −4) resolved the eye/visor z-fight headless Chrome found; the visor-rim speckle (visor↔head shell, same precision cause) was left as a known residual rather than chased further — real-device confirmation that it stays faint is this row |
